...
def ploting_shape_and_getting_charges(path):
    ...
    # Adjust the figure to make room for the text boxes
    plt.subplots_adjust(bottom=0.4)

    # TextBox 1 (Static Temperature)
    static_tempertaure_box_size = plt.axes([0.3, 0.30, 0.4, 0.05])  # [left, bottom, width, height]
    static_temperature_text_box = TextBox(static_tempertaure_box_size, 'Static Temperature: ', initial='')

    # TextBox 2 (Oscillating Temperature)
    dynamic_tempertaure_box_size = plt.axes([0.3, 0.20, 0.4, 0.05])  # [left, bottom, width, height]
    dynamic_temperature_text_box = TextBox(dynamic_tempertaure_box_size, 'Oscillating Temperature: ', initial='')

    # TextBox 3 (Oscillating Frequency)
    osci_box_size = plt.axes([0.3, 0.10, 0.4, 0.05])  # [left, bottom, width, height]
    osci_text_box = TextBox(osci_box_size, 'Oscillating Frequency: ', initial='')

    rows_static = []
    cols_static = []
    rows_osci = []
    cols_osci = []
    static_temp, dynamic_temp, osci_freq = [0], [0], [0] 

    def submit_stat_temp(text):
        static_temp[0] = float(text)
        static_temperature_text_box.set_val(f"Submitted: {text}")  
        fig.canvas.draw_idle()  
    
    def submit_osci_temp(text):
        dynamic_temp[0] = float(text)
        dynamic_temperature_text_box.set_val(f"Submitted: {text}")  
        fig.canvas.draw_idle()  

    def submit_freq(text):
        osci_freq[0] = float(text)
        osci_text_box.set_val(f"Submitted: {text}")  
        fig.canvas.draw_idle()  
        
    def onclick(event):
        '''
        click and creates the points
        input: event
    ...
    static_temperature_text_box.on_submit(submit_stat_temp)
    dynamic_temperature_text_box.on_submit(submit_osci_temp)
    osci_text_box.on_submit(submit_freq)

    plt.show()

    return cols_static, rows_static, cols_osci, rows_osci, static_temp[0], dynamic_temp[0], osci_freq[0]
...
cols_static, rows_static, cols_dynamic,rows_dynamic, tempature_static, tempetarure_dynamic, osci = ploting_shape_and_getting_charges(polygon_path)
